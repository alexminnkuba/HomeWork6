internal class Program
{
    ...
    private static Dictionary<int, int> passOnBus = new Dictionary<int, int>();
    ...
    static void GenerationBus()
    {
        while (flag)
        {
            resetEvent.WaitOne(); 
            int busNumber = busNumbers[random.Next(busNumbers.Count)];
            int currentPassengers = passOnBus.ContainsKey(busNumber) ? passOnBus[busNumber] : 0;
            int countPassToBus = Math.Min(passAtStop.ContainsKey(busNumber) ? passAtStop[busNumber] : 0, maxCountPassBus - currentPassengers);

            if (countPassToBus > 0)
            {
                lock (lockObj)
                {
                    if (!passOnBus.ContainsKey(busNumber))
                    {
                        passOnBus[busNumber] = currentPassengers;
                    }

                    passAtStop[busNumber] -= countPassToBus;
                    passOnBus[busNumber] += countPassToBus;
                    ...
                    Console.WriteLine($"Автобус № {busNumber} забрал {countPassToBus} пассажиров. " +
                        $"В салоне: {passOnBus[busNumber]}. Осталось на остановке: {TotalPass()}");
                }
            }
            else
            {
                Console.WriteLine($"Автобус № {busNumber} прибыл, но не смог забрать пассажиров. " +
                    $"В салоне: {passOnBus[busNumber]}. Осталось на остановке: {TotalPass()}");
            }
